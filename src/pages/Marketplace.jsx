          ...
          {filtered.length === 0 ? (
            ...
            <div className="row">
              {filtered.map((group, idx) => (
                <div className="col-lg-3 col-md-4 col-sm-6 mb-5" key={idx}>
                  <div className="featured__item" style={{ cursor: 'pointer' }} onClick={() => setSelectedGroup(group)}>
                    <div
                      className="featured__item__pic set-bg"
                      style={{ backgroundImage: `url(${group.imageUrl || feature1})` }}
                    >
                      <div style={{ position: 'absolute', top: '10px', left: '10px', background: 'var(--primary)', color: '#fff', padding: '4px 10px', borderRadius: '50px', fontSize: '11px', fontWeight: 700 }}>
                        {group.offers.length} Offer{group.offers.length > 1 ? 's' : ''}
                      </div>
                    </div>
                    <div className="featured__item__text">
                      <h6><a href="#" onClick={(e) => { e.preventDefault(); setSelectedGroup(group); }}>{group.name}</a></h6>
                      <h5>
